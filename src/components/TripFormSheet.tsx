      ...
      <div className="relative bg-surface rounded-t-[22px] shadow-sheet max-h-[93%] flex flex-col animate-sheet-up">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pt-4 pb-0">
          ...
          <div className="mb-5">
            ...
            {showEmojiPicker && (
              <div className="mt-3 bg-white rounded-xl border border-[#E4DFD9] p-3">
                {/* Section tabs */}
                <div className="flex gap-2 mb-3">
                  {(['travel', 'member'] as const).map(s => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-bold border ${emojiSection === s ? 'bg-primary text-white border-primary' : 'text-muted border-[#E4DFD9]'}`}
                    >
                      {s === 'travel' ? '旅遊' : '成員'}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {(emojiSection === 'travel' ? TRAVEL_EMOJIS : MEMBER_EMOJIS).map(e => (
                    <button
                      ...
                      className={`w-11 h-11 rounded-xl text-[22px] flex items-center justify-center border-[1.5px] transition-colors ${coverEmoji === e ? 'border-primary bg-[#FFF5F0]' : 'border-[#E4DFD9] bg-white'}`}
                    >
                      {e}
                    </button>
                  ))}
                ...
                <button
                  ...
                  className="w-full mt-3 py-2 bg-primary text-white text-sm font-bold rounded-xl"
                >
                  就用這個
                </button>
              </div>
            ...
            <div className="flex flex-col gap-2">
              {members.map((m, i) => (
                ...
                >
                  ...
                  <span className="flex-1 text-[16px] font-semibold text-ink">{m.name || '（未命名）'}</span>
                  {myMemberIdx === i && (
