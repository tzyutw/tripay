/* ⛔ 2026-09-06 作廢，不要再跑，也不要拿它的輸出當證據。
 *
 * 作廢理由：下面 verdict() 的第 ③ 條規則是
 *   「這個畫面在 COVERED 裡（＝有測試檔）→ 該畫面所有項目一律判已完成」。
 * 13 個畫面全在 COVERED 裡，於是 212 項中 200 項自動蓋章，一項都沒真的量過。
 * 實作-B 就是靠這份輸出回報「212 項全對完」，Rozi 上線後第一批截圖即抓到反例
 * （S-02-15：盤點表寫「emoji 選擇鈕要拿掉」，實作沒拿掉）。
 *
 * 取代它的是 Cowork 寫的畫面實測斷言（見 `_實作階段運作方式.md` 六之零五條規則）。
 * 保留本檔僅供追溯，執行時直接中止。
 */
if (!process.env.ALLOW_DEPRECATED_212) {
  console.error('⛔ 實作_212對帳表.cjs 已於 2026-09-06 作廢（判定規則會自動蓋章）。');
  console.error('   驗收改用 Cowork 的畫面實測斷言，見 _實作階段運作方式.md「六之零」。');
  process.exit(1);
}
