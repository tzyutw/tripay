...
/* 這些是**目前**的值；每一次變動都要在這裡寫清楚是誰造成的。
   ...
   實作-T：
     ...
     s03 +28　T-6 把底部距離從 16 改成 30：`.btnrow` 74→88，
       `.btnpad` 跟著 74→88。
   實作-X：
     s03 1632 → 1697（+65）　X-1 在統計卡與清單標頭之間加了一列
       「只看共同的帳」開關（`.swrow`）。
   實作-AC：
     s06 1419 → **1464**（+45）　AC-5 在分享頁的「誰付給誰」下面加了一顆
       「查看計算依據」（收合狀態）。**這是刻意加的一列**，實測 45 就是那顆按鈕的高度。
   實作-Z：
     s03 1697 → **1632**（−65）　Z-3 把那顆開關搬進「消費紀錄 · N 筆」那一行的
       右邊（Rozi：「按鈕有點太大了」），`.swrow` 整列移除。
       **差額剛好還原成 X 之前的 1632**，等於那一列的高度原封不動地還回去。

     s03 1632 → **1613**（−19）　收尾-AI-1 把外幣警告改短
     （「有 N 筆還沒換算成台幣，上面的總花費不含它們。」→「總花費還少了 N 筆，
     它們只填了外幣。」），390px 下從兩行變一行。實測那一句的高度是 19px、
     行高 18.85px、剛好一行——差額與頁高的變化對得起來，不是版面被誰擠掉。 */
const HEIGHT_BASE = { s01: 844, s02: 922, s02b: 1182, s03: 1613, s04: 783,
                      s05: 844, s06: 1464, s07: 844, s03more: 844 };
